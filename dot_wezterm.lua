...
config.keys = {
	...
	-- Split Panel (Ctrl+Shift+U/O like Ghostty)
	{
		key = "e",
		mods = "CTRL|SHIFT|ALT",
...
local function shell_path(cmd)
	if os == "windows" then
		local success, stdout, stderr = wezterm.run_child_process({ "where", cmd })
		if success and stdout and stdout ~= "" then
			local path = stdout:match("^%s*(.-)%s*$"):match("([^\r\n]*)")
			if path and path ~= "" then
				return path
			end
		end
	else
		local success, stdout, stderr = wezterm.run_child_process({ "which", cmd })
		if success and stdout and stdout ~= "" then
			local path = stdout:match("^%s*(.-)%s*$"):match("([^\r\n]*)")
			if path and path ~= "" then
				return path
		...
		for _, path in ipairs(common_paths) do
			local s = wezterm.run_child_process({ "test", "-x", path })
			if s then
				return path
			end
		end
